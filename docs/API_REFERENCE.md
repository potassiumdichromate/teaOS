...
## Confidential Compute Dashboard (read-only mirrors, Admin)

| Method | Path | Role | Notes |
|---|---|---|---|
| GET | `/compute/queue` | ADMIN | live validation queue depth/state |
| GET | `/compute/attestations/:reportId` | ADMIN | fetch/re-verify the stored TEE attestation for a validation report |
| GET | `/compute/audit-log` | ADMIN | paginated, filterable `AuditLog` |

## 0G Storage Explorer

| Method | Path | Role | Notes |
|---|---|---|---|
| GET | `/storage/objects` | ADMIN | list encrypted objects (Question/Paper/Answer blobs) with root hash, timestamp, node, never plaintext |
| GET | `/storage/objects/:root/proof` | ADMIN / public (for verification flow) | Merkle inclusion proof for a given root |
...
| GET | `/admin/blockchain-events` | ADMIN | paginated `ChainAnchor` + `MidenNote` |
...
| GET | `/admin/papers/:id/pipeline` | ADMIN | live stage: blueprint → TEE-validated pool → AI selection → master paper → hash → encryption → Miden timelock seal → ready |
...
| GET | `/student/exam/questions?sessionId=` | STUDENT | resume: re-derives the redacted question list for an already-IN_PROGRESS session from server-side cache, without re-consuming the Miden note |
| POST | `/student/exam/start` | STUDENT | `{sessionId}` — consumes the Miden P2IDE note to obtain the content key, decrypts the paper, returns the per-student randomized, answer-redacted question list. Genuinely refuses (409) until `Paper.status === READY` — there is no backend override key. |
...
| POST | `/student/exam/submit` | STUDENT | `{sessionId}` — aggregates cached answers, uploads the encrypted blob to 0G Storage, anchors on 0G Chain, creates a Miden private submission note |

## Evaluation Engine

| Method | Path | Role | Notes |
|---|---|---|---|
| POST | `/admin/evaluation/run` | ADMIN | `{paperId}` → enqueues evaluation job across all `SUBMITTED` sessions for that paper. Genuinely requires `Paper.status === READY` — same Miden gate as starting an exam. |
...
| Method | Path | Role | Notes |
|---|---|---|---|
| POST | `/verify` | public | `{applicationId, dob}` → itemized verification result (identity match, answer hash, submission hash, official result hash, Miden note/nullifier status, 0G Storage proof, 0G Chain tx) per knowledge_base.md §4 sequence 5 |
