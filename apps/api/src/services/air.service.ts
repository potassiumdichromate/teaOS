// AIR Ranking per docs/SYSTEM_ARCHITECTURE.md §8: scores -> sorting -> tie
// breaking -> AIR -> result hash. Purely a DB-level operation over already-
// computed EvaluationResult rows — no 0G/Miden calls of its own, so it's
// real and fully functional independent of the Miden blocker. It's only
// transitively blocked in the sense that there's nothing to rank until
...
// none scoped to "the AIR list as a whole"). Each student's individual
...
import { airRankingRepository } from "../repositories/airRanking.repository.js";
import { auditLogRepository } from "../repositories/auditLog.repository.js";
import { sha256Hex } from "../lib/crypto.js";
import { HttpError } from "../middleware/error.middleware.js";

export async function publishAIR(paperId: string, tieBreakRule: string) {
  ...
  await airRankingRepository.createMany(rows);
  await auditLogRepository.write("AIR_PUBLICATION", {
...
export async function getAIR(paperId: string) {
  return airRankingRepository.listForPaper(paperId);
}
