...
5. ~~Miden testnet funding~~ — moot as of 2026-07-28; Miden itself has been replaced by drand/tlock for the paper-key timelock (§11o/§11p). `contracts/miden/bridge/` stays in the repo, dormant, not deleted, and is still what the (also dormant) submission-commitment note path targets.
...
- [ ] **Submission-commitment notes still need a real, non-Miden-dependent replacement** — currently double-submission prevention is DB/app-level only (`session.status` guards), which is real but not protocol-level. §11o's own suggested design: "a simple commitment + nullifier mapping in a small contract on 0G Chain" (reuses the 0G Chain infra already anchoring everything else, rather than a second chain client). Until this exists, `verify.service.ts`'s `midenNoteValid`/`overallVerified` will correctly stay `false` for every session — do not loosen that check to hide the gap.
- [ ] Look into the two `ERROR`-severity `UntrustedMastForest` log lines that appear (non-fatal) during every Miden account provisioning — not yet investigated, low priority, and now only relevant to the dormant submission-note path.
