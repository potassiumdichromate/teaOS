...
## 0. Session Handoff (read this first — this session is ending, a new one is picking up)

**Read this whole section before doing anything else.** The user is switching Claude Code accounts due to hitting a weekly usage limit, so a brand-new session with no conversation memory is about to continue this project from this file alone. Do not re-derive or re-litigate anything below — it is settled.

**Project identity has changed.** This file's original framing (§1) — a government-facing NVEI prototype for NTA/JEE/NEET-class exams — is the *technical origin* of the project, not its current identity. The project is now pitched as **teaOS**, a B2B SaaS "operating system for examinations," built by **HorizonX Labs**, and is being prepared as a **YC Combinator submission**. Two founders: **Sidhanth K. Mahto** (infrastructure — blockchain 6+ yrs, DeAIOS 4 yrs, shipped decentralized products across major chains) and **Ray Tsai** (AI systems — agents, LLM infrastructure, full ML stack). The underlying codebase (`apps/`, `contracts/`, `prisma/`) is unchanged and still real — only the pitch framing and target audience changed, from "government reviewers" to "YC + enterprise/university/government buyers." Sections 1, 5 and similar government-audience framing further down are historically accurate for how the code was built but are no longer the external narrative — the pitch narrative lives in `pitch/`, not in this file's prose.

**The pitch one-pager is a first-class deliverable, not a side artifact.** It lives at `pitch/teaos-onepager.template.html` (source) → `pitch/build.js` (injects compressed assets from `assets/opt/`) → `pitch/index.html` (standalone document, served by Vercel with root directory `pitch`) and `pitch/artifact.html` (fragment, used for the claude.ai Artifact preview only — hosts its own `<head>`). **Always edit the template and re-run `node build.js` from inside `pitch/` — never hand-edit `index.html` or `artifact.html` directly, they're generated.** The whole repo is pushed to `https://github.com/potassiumdichromate/teaOS` (public), which is what Vercel deploys from and what the cloud compatibility-check routine (below) clones.

**Current top-priority engineering decision: replacing the Miden timelock with drand/tlock.** See §11o for the full record — short version: Miden's public testnet has rejected our client (`miden-client` 0.16.0-alpha.1, the newest version that exists) for going on two days now, confirmed via an hourly automated cloud check (still running — see §11o for the trigger ID). The user has decided to stop waiting and replace the timelock mechanism with **drand/tlock**, a mature, production-grade decentralized timelock-encryption network, rather than depending further on Miden's early-alpha SDK and infrastructure rollout timeline. **This has been decided but not yet implemented** — no tlock code exists yet. This is the next concrete engineering task. Do not re-relitigate whether to switch; the decision is made. Do research the real `tlock`/`drand` Rust (or other) client crates against actual source before writing anything — same discipline used for every Miden fix in this project (verify via source, not docs.rs summaries or memory).

**Do not delete the Miden bridge code.** Keep `contracts/miden/bridge/` in place, dormant, in case Miden's testnet becomes usable later and the user wants both backends. The kernel abstraction the pitch already describes (`seal(payload, until_block)`, `schedule(window)`) is deliberately backend-agnostic — implement tlock behind that same interface shape rather than bolting it on separately.

**What to do immediately in the new session:** `git pull` the repo (or start fresh from GitHub if the local clone isn't present — check `C:\Users\RENTKAR\Desktop\0g-ai\nta` first), read this file fully, then read §11o below for the full technical handoff on the tlock migration, then begin researching real tlock/drand crates. If asked to touch the pitch one-pager, remember the template → build.js → index.html/artifact.html pipeline above.
...
5. ~~Miden testnet funding~~ — moot as of 2026-07-28; the asset-requirement work is done (§11n) but Miden itself is being replaced by drand/tlock (§11o), so no further Miden credentials are needed. `contracts/miden/bridge/` stays in the repo, dormant, not deleted.
...
- [x] ~~Blocked on Miden's own infrastructure~~ — superseded. After ~2 days / ~40 hourly automated checks with no unblock, the user decided (2026-07-28) to stop waiting on Miden and replace the timelock mechanism with **drand/tlock** instead. **This is now the top-priority open task.** Full rationale, what changes, and concrete next steps: see §11o. Do not re-litigate the decision; do research the real tlock/drand libraries via source before implementing.
- [ ] **Implement drand/tlock as the new timelock backend** (see §11o steps 1–5 for the full sequence: research real crates → decide runtime/language → prove a standalone encrypt/decrypt round-trip → wire behind the existing call site → re-run the full pipeline live → update the pitch one-pager's technical claims to match).
- [ ] Once tlock is live: re-run Paper Generation to reach a real `READY` state with a real timelock reference, then re-run Student Exam Client / Evaluation Engine / Student Verification against that real paper end-to-end.
